...
happyExpList = Happy_Data_Array.listArray (0,41) ([33760,256,0,1,15872,31752,16,16,32,81,0,16,512,0,1024,8440,16880,0,0,3968,2,0,0,8,61440,65,8,32768,527,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseMDL","MDL","Condition","right","left","forward","obstacle","if","then","else","boolVal","boolOp","int","done","%eof"]
        bit_start = st * 17
        bit_end = (st + 1) * 17
...
action_5 (4) = happyGoto action_16
...
happyReduction_1 (HappyAbsSyn4  happy_var_3)
	(HappyTerminal (TokenMDLVar _ happy_var_2))
	_
	 =  HappyAbsSyn4
		 (Forward happy_var_2 happy_var_3
	)
happyReduction_1 _ _ _  = notHappyAtAll 

happyReduce_2 = happySpecReduce_2  4 happyReduction_2
happyReduction_2 (HappyTerminal (TokenMDLVar _ happy_var_2))
	_
	 =  HappyAbsSyn4
		 (Forward happy_var_2 End
	)
...
happyReduction_7 (HappyAbsSyn4  happy_var_3)
	...
	_
	 =  HappyAbsSyn4
		 (Obstacle happy_var_2 happy_var_3
	)
happyReduction_7 _ _ _  = notHappyAtAll 

happyReduce_8 = happySpecReduce_2  4 happyReduction_8
happyReduction_8 (HappyTerminal (TokenMDLVar _ happy_var_2))
	_
	 =  HappyAbsSyn4
		 (Obstacle happy_var_2 End
	)
...
happyReduction_11 (HappyTerminal (TokenMDLBoolVal _ happy_var_1))
	 =  HappyAbsSyn5
		 (BoolVal happy_var_1
	)
...
happyReduction_12 (HappyTerminal (TokenMDLVar _ happy_var_3))
	(HappyTerminal (TokenMDLBoolOp _ happy_var_2))
	(HappyTerminal (TokenMDLVar _ happy_var_1))
	 =  HappyAbsSyn5
		 (BoolOperation happy_var_2 happy_var_1 happy_var_3
	)
happyReduction_12 _ _ _  = notHappyAtAll 

happyReduce_13 = happySpecReduce_2  5 happyReduction_13
happyReduction_13 (HappyTerminal (TokenMDLVar _ happy_var_2))
	_
	 =  HappyAbsSyn5
		 (Obstacle happy_var_2 End
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	TokenRotateRight _ -> cont 6;
	TokenRotateLeft _ -> cont 7;
	TokenForward _ -> cont 8;
	TokenObstacle _ -> cont 9;
	TokenIf _ -> cont 10;
	TokenThen _ -> cont 11;
	TokenElse _ -> cont 12;
	TokenMDLBoolVal _ happy_dollar_dollar -> cont 13;
	TokenMDLBoolOp _ happy_dollar_dollar -> cont 14;
	TokenMDLVar _ happy_dollar_dollar -> cont 15;
	TokenMDLEnd _ -> cont 16;
	_ -> happyError' ((tk:tks), [])
	}
...
parseMDL tks = happyRunIdentity happySomeParser where
